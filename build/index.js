...
// 5. 日期运算模块实现 - 日期加减
function dateArithmetic(datetime, years, months, days, hours, minutes, seconds, format) {
...
// 6. 日期运算模块实现 - 日期差值
function dateDifference(datetime1, datetime2, unit) {
...
// 4. 随机数模块实现
function generateRandomNumbers(min, max, count, decimalPlaces) {
    const results = [];
...
// ============ 注册工具 ============
// 1. 计算工具 - 表达式计算
mcpServer.registerTool("calculate_expression", {
...
// 2. 计算工具 - 单位换算
mcpServer.registerTool("unit_conversion", {
...
// 2. 日期与时间工具
mcpServer.registerTool("get_current_datetime", {
...
// 3. 时间戳转换工具
mcpServer.registerTool("timestamp_conversion", {
...
// 5. 日期运算工具 - 日期加减
mcpServer.registerTool("date_arithmetic", {
    description: "日期加减运算，支持对日期进行年、月、日、时、分、秒的加减操作",
    inputSchema: DateArithmeticSchema,
}, async (input) => {
    const result = dateArithmetic(input.datetime, input.years || 0, input.months || 0, input.days || 0, input.hours || 0, input.minutes || 0, input.seconds || 0, input.format || 'yyyy-MM-dd HH:mm:ss');
    return {
        content: [
            {
                type: "text",
...
// 6. 日期运算工具 - 日期差值
mcpServer.registerTool("date_difference", {
    description: "计算两个日期之间的差值，支持按天、小时、分钟、秒为单位返回",
    inputSchema: DateDifferenceSchema,
}, async (input) => {
    const result = dateDifference(input.datetime1, input.datetime2, input.unit || 'days');
    return {
        content: [
            {
                type: "text",
...
// 7. 随机数生成工具
mcpServer.registerTool("generate_random_numbers", {
    description: "生成随机数，支持定义最大最小、生成数量、保留几位小数",
    inputSchema: RandomNumberInputSchema,
}, async (input) => {
    const result = generateRandomNumbers(input.min, input.max, input.count, input.decimalPlaces);
    return {
        content: [
            {
                type: "text",
